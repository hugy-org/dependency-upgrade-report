...
data class GitHubConfig(
    ...
    init {
        require(maxReleases > 0) { "maxReleases must be above 0" }
        require(pageSize > 0) { "pageSize must be above 0" }
        require(maxScanReleases > 0) { "maxScanReleases must be above 0" }
    }
}
...
data class LlmConfig(
    ...
    fun resolveApiKey(environment: Map<String, String> = System.getenv()): String? {
        ...
        val envName = apiKeyEnv?.takeIf { it.isNotBlank() } ?: return null
        return environment[envName]?.takeIf { it.isNotBlank() }
    }
}
